...
void setup() {
  ...
  /* ST's VL53L1X_SensorInit: one throwaway ranging cycle, then the VHV
   * settings, then start for real. Skipping the middle step leaves the part
   * identified and booted but never producing a measurement. */
  tofWrite8(0x0087, 0x40);                              /* start ranging   */
  for (int i = 0; i < 200 && (tofRead(0x0031, 1) & 0x01) == 0; i++) delay(5);
  tofWrite8(0x0086, 0x01);                              /* clear interrupt */
  tofWrite8(0x0087, 0x00);                              /* stop ranging    */
  tofWrite8(0x0008, 0x09);                              /* VHV config      */
  tofWrite8(0x000b, 0x00);
  tofWrite8(0x0087, 0x40);                              /* start ranging   */
